# honesty-dotnet
`honesty-dotnet` is a collection of lightweight [monads](https://en.wikipedia.org/wiki/Monad_(functional_programming)) - `Optional<T>` and `Result<T>`, few simple extension methods of `Task<T>`, and `Unit` type which indicates the absence of a specific value generated by an expression (think void in OOP). These constructs can be used to convert pure functions to honest functions, write LINQ queries on even non-sequence data types and chain powerful functional compositions.
